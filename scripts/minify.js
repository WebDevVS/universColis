...
const cookiesBannerDir = path.join(__dirname, '../static/cookiesBanner');
...
// Minify JS in cookiesBanner
fs.readdirSync(cookiesBannerDir).forEach(file => {
  if (file.endsWith('.js') && !file.endsWith('.min.js')) {
    const filePath = path.join(cookiesBannerDir, file);
    const code = fs.readFileSync(filePath, 'utf8');
    minify(code, { compress: true, mangle: true }).then(result => {
      const outPath = filePath.replace('.js', '.min.js');
      fs.writeFileSync(outPath, result.code, 'utf8');
